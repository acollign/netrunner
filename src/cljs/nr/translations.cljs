...
(def translation-dictionary
  ...
  :diagrams
  {
   :run-timing.initiation "6.9.1. : Phase d’Initialisation"
   :run-timing.initiation-a "Lae Runner déclare le serveur attaqué"
   :run-timing.initiation-b "Lae Runner gagne les crédits de Mauvaise Presse"
   :run-timing.initiation-c "Le piratage commence formellement. Résolution des effets qui se déclenchent à cette étape"
   :run-timing.initiation-d "S’il y a une glace périphérique, commencez la phase d’approche (6.9.2.)"
   :run-timing.initiation-e "Sinon, continuez vers la phase de mouvement (6.9.4.)"
   :run-timing.approach "6.9.2. : Phase d’Approche de la Glace"
   :run-timing.approach-a "Vous êtes en train d’approcher la glace. Résolution des effets qui se déclenchent à cette étape"
   :run-timing.approach-b "Fenêtre de capacité payante. La corpo peut activer la glace en cours d’approche ou toutes autres cartes excepté des glaces"
   :run-timing.approach-c "Si la glace en cours d’approche est activée, continuez vers la phase de rencontre (6.9.3.)"
   :run-timing.approach-d "Sinon, continuez vers la phase de mouvement (6.9.4.)"
   :run-timing.disclaimer "Ces étapes ont été simplifiées pour plus de clarté. Pour une description complète, rendez-vous sur le site de Null Signal Games. "
   :run-timing.encounter "6.9.3. : Phase de Rencontre de la Glace"
   :run-timing.encounter-a "Vous rencontrez maintenant cette glace. Résolution des effets qui se déclenchent à cette étape"
   :run-timing.encounter-b "Fenêtre de capacité payante. Les brise-glaces peuvent s’interfacer avec la glace en cours de rencontre "
   :run-timing.encounter-c "S’il reste des routines non neutralisées, la corpo résout celle qui est la plus proche du haut de la carte. Répétez cette opération autant de fois que nécessaire"
   :run-timing.encounter-d "La rencontre est terminée. Passez à la phase de mouvement (6.9.4.)"
   :run-timing.header "Structure Chronologique d’un Piratage"
   :run-timing.movement "6.9.4. : Phase de Mouvement"
   :run-timing.movement-a "Si vous avez approché ou rencontré une glace, vous la passez. Résolution des effets qui se déclenchent à cette étape"
   :run-timing.movement-b "S’il n’y a pas d’autre glace après celle qui vient d’être passée, résolvez les effets qui se déclenchent lorsque toutes les glaces sur un serveur ont été passées "
   :run-timing.movement-c "Fenêtre de capacité payante"
   :run-timing.movement-d "Lae runner peut débrancher. Dans ce cas, passez à la phase de fin de piratage (6.9.6.)"
   :run-timing.movement-e "Lae runner continue, le cas échéant, vers la glace suivante"
   :run-timing.movement-f "Fenêtre de capacité payante. La corpo peut activer toutes cartes, excepté des glaces"
   :run-timing.movement-g "S’il y a une autre glace, retournez à la phase d’approche (6.9.2.)"
   :run-timing.movement-h "Lae runner approche le serveur attaqué. Résolution des effets qui se déclenchent à cette étape"
   :run-timing.movement-i "Continuez vers la phase de réussite (6.9.5.)"
   :run-timing.success "6.9.5. : Phase de Réussite"
   :run-timing.success-a "Le piratage est déclaré réussi. Résolution des effets déclenchés à cette étape"
   :run-timing.success-b "Lae runner fracture le serveur attaqué"
   :run-timing.success-c "La phase de réussite prend fin. Continuez vers la phase de fin de piratage (6.9.6.)"
   :run-timing.run-ends "6.9.6. : Phase de Fin de Piratage"
   :run-timing.run-ends-a "Les fenêtres de priorité ouvertes sont résolues ou fermées."
   :run-timing.run-ends-b "Lae runner perd les crédits de mauvaise presse non dépensés"
   :run-timing.run-ends-c "Si la phase de succès n’a pas été atteinte et si le serveur existe encore, le piratage est déclaré raté"
   :run-timing.run-ends-d "Le piratage prend fin. Résolution des effets qui se déclenchent à cette étape"
   :turn.corp-draw-phase "5.6.1. : Phase de Pioche"
   :turn.corp-draw-phase-a "La corpo gagne les clics alloués pour ce tour (par défaut : [click][click][click]) "
   :turn.corp-draw-phase-b "Fenêtre de capacité payante. La corpo peut activer toutes cartes (excepté des glaces) ou finaliser des projets"
   :turn.corp-draw-phase-c "Les crédits récurrents de la corpo sont renouvelés"
   :turn.corp-draw-phase-d "Le tour commence formellement. Résolution des effets qui se déclenchent à cette étape"
   :turn.corp-draw-phase-e "La corpo effectue sa pioche obligatoire"
   :turn.corp-draw-phase-f "Passez à la phase d’action (5.6.2.)"
   :turn.corp-action-phase "5.6.2. : Phase d’Action"
   :turn.corp-action-phase-a "Fenêtre de capacité payante. La corpo peut activer toutes cartes (excepté des glaces) ou finaliser des projets"
   :turn.corp-action-phase-b "Si la corpo a des [Clicks] non dépensés, elle effectue une action"
   :turn.corp-action-phase-c "Si une action a été effectuée, retournez à l’étape (a)"
   :turn.corp-action-phase-d "La phase d’action est terminée. Passez à la phase de défausse (5.6.3.)"
   :turn.corp-discard-phase "5.6.3. : Phase de Défausse"
   :turn.corp-discard-phase-a "Le cas échéant, la corpo se défausse du nombre de cartes qui excède la taille maximale de sa main"
   :turn.corp-discard-phase-b "Fenêtre de capacité payante. La corpo peut activer toutes cartes, excepté des glaces"
   :turn.corp-discard-phase-c "Si la corpo a encore des [Clicks], elle les perd"
   ...
   :turn.runner-action-phase-c "Les crédits récurrents de lae runner sont renouvelés"
   :turn.runner-action-phase-d "Le tour commence formellement. Résolution des effets qui se déclenchent à cette étape"
   :turn.runner-action-phase-e "Fenêtre de capacité payante. La corpo peut activer toutes cartes, excepté des glaces"
   :turn.runner-action-phase-f "Si lae runner a des [Clicks] non dépensés, iel effectue une action"
   :turn.runner-action-phase-g "Si une action a été effectuée, retournez à l’étape (e)"
   :turn.runner-action-phase-h "La phase d’action est terminée. Passez à la phase de défausse (5.7.2.)"
   :turn.runner-discard-phase "5.7.2. : Phase de défausse"
   :turn.runner-discard-phase-a "Le cas échéant, lae runner se défausse du nombre de cartes qui excède la taille maximale de sa main"
   :turn.runner-discard-phase-b "Fenêtre de capacité payante. La corpo peut activer toutes cartes, excepté des glaces"
   :turn.runner-discard-phase-c "Si lae runner a des [Clicks"
   :turn.runner-discard-phase-d "Le tour de lae runner prend formellement fin. Résolution des effets qui se déclenchent à cette étape"
   :turn.runner-discard-phase-e "Passez au tour de la corpo"}
   }
